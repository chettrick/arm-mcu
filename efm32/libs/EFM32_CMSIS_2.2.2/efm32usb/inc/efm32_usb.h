 ...
 *
 ******************************************************************************/
#ifndef __EFM32_USB_H
#define __EFM32_USB_H
 ...
 *        and USBHOST pages for device and host library documentation.
 * @{
 ******************************************************************************/
...
/** Macro for creating USB compliant UNICODE string descriptors.
...
/** Macros for creating WORD (4 byte) aligned static uint8_t arrays with size which
 *  is a multiple of WORD size.
 *  @n Example: @n UBUF( rxBuffer, 37 );  =>  uint8_t rxBuffer[ 40 ];
 *              @n STATIC_UBUF( rxBuffer, 37 );  =>  static uint8_t rxBuffer[ 40 ];
 */
#if !defined(__GNUC__)
...
typedef enum
{
  ...
  USB_STATUS_TIMEOUT         = -11,             /**< Transfer timeout.                                 */
  USB_STATUS_DEVICE_REMOVED  = -12,             /**< Device was removed.                               */
...
typedef struct
{
  ...
  uint8_t  bMaxPacketSize0;                     /**< Number Maximum packet size for endpoint zero      */
...
typedef struct
{
  ...
  uint8_t bInterfaceNumber;                     /**< Number Number of this interface. Zero-based value */
  ...
  uint8_t bInterfaceClass;                      /**< Class Class code (assigned by the USB-IF). A value*/
...
typedef struct
{
  ...
  wchar_t name[];                               /**< The string encoded as UNICODE                     */
 ...
 *
 * @details
 *  The callback function is called when an USBTIMER has elapsed.
 ******************************************************************************/
 ...
 *
 * @return
 *  An approriate status/error code. See @ref USB_Status_TypeDef.
 ******************************************************************************/
 ...
 *  This structure is passed to @ref USBD_Init() when starting up the device. */
typedef struct
{
 ...
 *  Callback functions used by the device stack to signal events or query
 *  status to/from the application. See @ref USBD_Init_TypeDef. Assign
...
typedef struct
{
  ...
  struct tag_USBH_Device_TypeDef  *parentDevice;  /**< The device the endpoint belongs to.              */
 ...
 *  A host application should not manipulate the contents of this struct. */
typedef struct tag_USBH_Device_TypeDef
{
 ...
 *  This structure is passed to @ref USBH_Init() when starting up the device.
 *  Max accumulated FIFO size is 2K bytes.                                    */
typedef struct
{
  uint32_t                        rxFifoSize;     /**< Number of bytes set aside for IN endpoints.            */
  uint32_t                        nptxFifoSize;   /**< Number of bytes set aside for OUT CTRL/BULK endoints.  */
  uint32_t                        ptxFifoSize;    /**< Number of bytes set aside for OUT INTR/ISO endoints.   */
} USBH_Init_TypeDef;
...
{                                                                       \
  1024,                     /* 1024 bytes Rx FIFO size.             */  \
  512,                      /* 512 bytes non-periodic Tx FIFO size. */  \
  512,                      /* 512 bytes periodic Tx FIFO size.     */  \
}

/*** -------------------- HOST mode init functions ----------------------- ***/

int   USBH_AssignHostChannel(           USBH_Ep_TypeDef *ep, uint8_t hcnum );
int   USBH_Init(                        const USBH_Init_TypeDef *p );
void  USBH_InitDeviceData(              USBH_Device_TypeDef *device, const uint8_t *buf, USBH_Ep_TypeDef *ep, int numEp, uint8_t deviceSpeed );
void  USBH_Stop(                        void );

/*** -------------------- HOST mode transfer functions ------------------- ***/

int   USBH_ControlMsgAsync(             USBH_Ep_TypeDef *ep, uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength, void *data, int timeout, USB_XferCompleteCb_TypeDef callback );
int   USBH_ControlMsgSync(              USBH_Ep_TypeDef *ep, uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength, void *data, int timeout );
int   USBH_ReadAsync(                   USBH_Ep_TypeDef *ep, void *data, int byteCount, int timeout, USB_XferCompleteCb_TypeDef callback );
int   USBH_ReadSync(                    USBH_Ep_TypeDef *ep, void *data, int byteCount, int timeout );
int   USBH_WriteAsync(                  USBH_Ep_TypeDef *ep, void *data, int byteCount, int timeout, USB_XferCompleteCb_TypeDef callback );
int   USBH_WriteSync(                   USBH_Ep_TypeDef *ep, void *data, int byteCount, int timeout );

/*** -------------------- HOST mode chapter 9 control functions ---------- ***/

int   USBH_GetConfigurationDescriptor(  USBH_Device_TypeDef *device, void *buf, int len);
int   USBH_GetDeviceDescriptor(         USBH_Device_TypeDef *device, void *buf, int len);
int   USBH_GetString(                   USBH_Device_TypeDef *device, uint8_t *buf, uint8_t stringIndex );
int   USBH_SetAddress(                  USBH_Device_TypeDef *device, uint8_t deviceAddress );
int   USBH_SetAltInterface(             USBH_Device_TypeDef *device, uint8_t interfaceIndex, uint8_t alternateSetting );
int   USBH_SetConfiguration(            USBH_Device_TypeDef *device, uint8_t configValue );
int   USBH_StallEp(                     USBH_Device_TypeDef *device, USBH_Ep_TypeDef *ep );
int   USBH_UnStallEp(                   USBH_Device_TypeDef *device, USBH_Ep_TypeDef *ep );

/*** -------------------- HOST mode port control ------------------------- ***/

uint8_t USBH_GetPortSpeed(              void );
bool    USBH_PortConnected(             void );
int     USBH_PortReset(                 void );
int     USBH_PortResume(                void );
void    USBH_PortSuspend(               void );
int     USBH_PortVbusOn(                bool on );

/*** -------------------- HOST mode enumeration/query functions ---------- ***/

int                                  USBH_QueryDevice(                 uint8_t *buf, size_t bufsize, uint8_t deviceSpeed );
USB_ConfigurationDescriptor_TypeDef* USBH_QGetConfigurationDescriptor( const uint8_t *buf, int confIndex );
USB_DeviceDescriptor_TypeDef*        USBH_QGetDeviceDescriptor(        const uint8_t *buf );
USB_EndpointDescriptor_TypeDef*      USBH_QGetEndpointDescriptor(      const uint8_t *buf, int confIndex, int interfaceIndex, int endpointIndex );
USB_InterfaceDescriptor_TypeDef*     USBH_QGetInterfaceDescriptor(     const uint8_t *buf, int confIndex, int interfaceIndex );

/*** -------------------- HOST mode console print utilities -------------- ***/

void  USBH_PrintString(                  const char *pre, const USB_StringDescriptor_TypeDef *s, const char *post );

#if defined( USB_USE_PRINTF )
void  USBH_PrintConfigurationDescriptor( const USB_ConfigurationDescriptor_TypeDef *config, int maxLen );
void  USBH_PrintDeviceDescriptor(        const USB_DeviceDescriptor_TypeDef *device );
void  USBH_PrintEndpointDescriptor(      const USB_EndpointDescriptor_TypeDef *endpoint );
void  USBH_PrintInterfaceDescriptor(     const USB_InterfaceDescriptor_TypeDef *interface );
