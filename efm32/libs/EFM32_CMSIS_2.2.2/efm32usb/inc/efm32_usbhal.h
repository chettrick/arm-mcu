...
static __INLINE void USBHAL_FlushRxFifo( void )
{
  ...
  while ( reset & USB_GRSTCTL_RXFFLSH );
...
static __INLINE void USBHAL_FlushTxFifo( uint8_t fifoNum )
{
  ...
  reset  = USB_GRSTCTL_TXFFLSH;
  reset |= fifoNum << _USB_GRSTCTL_TXFNUM_SHIFT;
  USB->GRSTCTL = reset;
  ...
  while ( reset & USB_GRSTCTL_TXFFLSH );
...
#if defined( USB_DEVICE )
static __INLINE void USBDHAL_ActivateEp( USBD_Ep_TypeDef *ep )
{
                                       ...
                                       USBH_Hc_TypeDef *hc )
{
  ...
  if ( hc->remaining > 0 )
  {
         ...
         ( ( fifoStatus & _USB_GNPTXSTS_NPTXFSPCAVAIL_MASK ) >= words )    )
    {
      ...
      hc->txFempIntOn = false;                /* Current package processed  */
    }
    else
    {
