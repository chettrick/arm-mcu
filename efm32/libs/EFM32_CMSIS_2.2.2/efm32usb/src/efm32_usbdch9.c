/**************************************************************************//**
 * @file
 * @brief USB protocol stack library, USB chapter 9 command handler for EFM32.
...
static USB_Status_TypeDef ClearFeature     ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef GetConfiguration ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef GetDescriptor    ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef GetInterface     ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef GetStatus        ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef SetAddress       ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef SetConfiguration ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef SetFeature       ( USBD_Device_TypeDef *dev );
static USB_Status_TypeDef SetInterface     ( USBD_Device_TypeDef *dev );
  ...
  switch ( p->Recipient )
  {
    case USB_SETUP_RECIPIENT_DEVICE:
             ...
             ( dev->state == USBD_STATE_CONFIGURED    )    )    )
      {
        ...
        if ( dev->configDescriptor->bmAttributes & CONFIG_DESC_BM_REMOTEWAKEUP )
          {
...
static USB_Status_TypeDef GetConfiguration( USBD_Device_TypeDef *dev )
{
  ...
  if ( dev->state == USBD_STATE_ADDRESSED )
  {
    *pConfigValue = 0;
  ...
  else if ( dev->state == USBD_STATE_CONFIGURED )
  {
    USBD_Write( 0, &dev->configurationValue, 1, NULL );
    retVal = USB_STATUS_OK;
  }
...
static USB_Status_TypeDef GetDescriptor( USBD_Device_TypeDef *dev )
{
  ...
  switch ( p->wValue >> 8 )
  {
    case USB_DEVICE_DESCRIPTOR :
      ...
      data   = dev->deviceDescriptor;
      length = dev->deviceDescriptor->bLength;
      break;
    ...
    case USB_CONFIG_DESCRIPTOR :
      ...
      data   = dev->configDescriptor;
      length = dev->configDescriptor->wTotalLength;
      break;

    case USB_STRING_DESCRIPTOR :
      if ( index < dev->numberOfStrings )
      {
        USB_StringDescriptor_TypeDef *s;
        s = ((USB_StringDescriptor_TypeDef**)dev->stringDescriptors)[index];
...
static USB_Status_TypeDef GetInterface( USBD_Device_TypeDef *dev )
{
  ...
  if ( dev->state == USBD_STATE_CONFIGURED )
  {
    *pAlternateSetting = 0;
    ...
    case USB_SETUP_RECIPIENT_DEVICE:
             ...
             ( dev->state == USBD_STATE_CONFIGURED )    )    )
      {
        *pStatus = 0;
        ...
        if ( dev->remoteWakeupEnabled )
          *pStatus |= REMOTE_WAKEUP_ENABLED;

        /* Current self/bus power status */
        if ( dev->callbacks->isSelfPowered != NULL )
        {
          if ( dev->callbacks->isSelfPowered() )
          {
        ...
        {
          if ( dev->configDescriptor->bmAttributes & CONFIG_DESC_BM_SELFPOWERED )
          {
            *pStatus |= DEVICE_IS_SELFPOWERED;
          }
        }
...
static USB_Status_TypeDef SetConfiguration( USBD_Device_TypeDef *dev )
{
  ...
  {
    if ( ( p->wValue == 0                                          ) ||
         ( p->wValue == dev->configDescriptor->bConfigurationValue )    )
    {
      dev->configurationValue = p->wValue;
      if ( p->wValue == dev->configDescriptor->bConfigurationValue )
      {
        USBD_ActivateAllEps();
