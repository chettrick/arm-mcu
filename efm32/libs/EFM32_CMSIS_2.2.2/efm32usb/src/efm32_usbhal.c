/**************************************************************************//**
 * @file
 * @brief USB protocol stack library, low level USB peripheral access for EFM32.
...
{
  uint32_t reset, timeout;

  /* Wait for AHB master IDLE state. */
  timeout = 0;
  do
  {
    __NOP(); __NOP(); __NOP();              /* Wait for 3 PHY Clocks */
    reset = USB->GRSTCTL;
  ...
  while ( !( reset & USB_GRSTCTL_AHBIDLE ) );

  /* Core Soft Reset */
  reset |= USB_GRSTCTL_CSFTRST;
  USB->GRSTCTL = reset;
  ...
  while ( reset & USB_GRSTCTL_CSFTRST );
                                     ...
                                     uint32_t totalTxFifoSize )
{
  ...
  USB->PCGCCTL = 0;                       /* Restart the Phy Clock */
  USB->DCFG &= ~_USB_DCFG_PERFRINT_MASK;  /* Periodic frame interval at 80% */
...
#if defined( USB_SLAVEMODE )
    /* Wait for Rx FIFO filled with global OUT NAK status */
    while ( !( USB->GINTSTS & USB_GINTSTS_RXFLVL ) ) {}
    USB->GRXSTSP;                           /* Pop status from top of FIFO */
#endif
                                     ...
                                     const uint32_t ptxFifoSize )
{
  ...
  if ( ( rxFifoSize + nptxFifoSize + ptxFifoSize ) > ( MAX_FIFO_SIZE * 4 ) )
  {
    return USB_STATUS_ILLEGAL;
  }
...
void USBHHAL_HCHalt( uint8_t hcnum, uint32_t hcchar, uint32_t eptype )
{
...
void USBHHAL_HCInit( uint8_t hcnum )
{
  USBH_Ep_TypeDef *ep;
...
void USBHHAL_HCStart( uint8_t hcnum )
{
  USBH_Hc_TypeDef *hc;
  ...
  USB->HC[ hcnum ].CHAR =
                    ( USB->HC[ hcnum ].CHAR & ~USB_HC_CHAR_CHDIS ) |
                    USB_HC_CHAR_CHENA;

  if ( !hc->ep->in )
  {
    /* NOTE: USB->GNPTXSTS is only correct for CTRL/BULK ep's !!!! */
