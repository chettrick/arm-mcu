...
void USBD_SetUsbState( USBD_State_TypeDef newState )
{
  ...
  {
    if ( ( currentState == USBD_STATE_POWERED    ) ||
         ( currentState == USBD_STATE_DEFAULT    ) ||
         ( currentState == USBD_STATE_ADDRESSED  ) ||
         ( currentState == USBD_STATE_CONFIGURED )    )
    {
...
@page usb_device USB device stack library
  ...
  The source files for the USB device stack resides in the efm32usb directory
  and follows the naming convention: efm32_usbd<em>nnn</em>.c/h.
...
@n @section usb_intro Introduction

  The EFM32 USB device protocol stack provides an API which makes it possible to
...
@n @section usb_api The device stack API
  ...
  Your application code must include one header file: @em efm32_usb.h.
  ...
  @n @anchor TransferCallback <b>The transfer complete callback function:</b> @n
    ...
    @endhtmlonly calls.
    @note This callback is called from within an interrupt handler.
...
@n @section usb_conf Configuring the stack
...
#define USER_PUTCHAR  RETARGET_WriteChar @endverbatim
  ...
  in both @em efm32lib and in the USB stack. If asserts are enabled and
...
@n @section usb_example1 Vendor unique device example application
...
static uint8_t bufferingMultiplier[ NUM_EP_USED + 1 ] = { 1 };
